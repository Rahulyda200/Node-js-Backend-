...
exports.register = async (req, res) => {
  try {
    ...
    const { user, token } = await userService.registerUser({ name, email, phone, address, password });
...
exports.updateUserById = async (req, res) => {
  try {
    ...
    const updatedUser = await userService.updateUserById(userId, { ...userUpdates, password });
    res.status(200).json({
      success: true,
